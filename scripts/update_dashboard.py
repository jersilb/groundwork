...
def render_metrics_table(status):
    ...
    failed = status.get("gates_failed", [])
    failed_str = ", ".join(str(p) for p in failed) if failed else "none"
    ...
    rows = [
        ...
        f"| Gates measured and FAILED | Phase(s) {failed_str} | 🔴 | {status['updated_at']} |",
