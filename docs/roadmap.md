...
**Update, 2026-07-31 — the precision number was found to be non-reproducible, and the obvious fix doesn't work on this model**: re-running the identical harness against identical fixtures produced 0.667, then 0.750, then 0.500 across separate runs — genuine sampling randomness, since `AnthropicLlmClient` never set `temperature`. Jeremy approved setting `temperature: 0`; **`claude-sonnet-5` rejects the parameter outright** (`"temperature is deprecated for this model"`, HTTP 400 on all 16 cases) — reverted the same session. A real baseline re-measurement post-revert: **precision 0.750, recall 0.500** — still fails 0.8, and there is no known determinism control available on this model. See `docs/decisions.md`, 2026-07-31.
...
## Phase 7 — Commerce and PWA 🟡 BUILT — PWA sub-gate AND commerce sub-gate both PASSED for real (2026-07-31)
...
**What's actually verified — the commerce half, now also for real (2026-07-31)**: pricing math and Stripe webhook signature verification (unchanged, deterministic). **Once this session teleported to Jeremy's own machine** (normal outbound network access, unlike the cloud sandbox), `scripts/test-phase7-commerce-live.mjs` ran real Stripe test-mode API calls end to end against a local `wrangler dev --local` instance (D1/R2/Queue emulated locally, no billed Cloudflare resources touched): a real checkout session (`cs_test_...`, a genuine `checkout.stripe.com` URL), a real Stripe customer created and deleted, and a real billing portal session (`billing.stripe.com` URL). The Stripe SDK is wired for Workers (`Stripe.createFetchHttpClient()`, no Node runtime dependency).
**What's still NOT verified**: a human actually completing checkout in a real browser, and Stripe's webhook firing from Stripe's own infrastructure (needs a public URL or `stripe listen`, neither set up yet). The literal "install to home screen on iOS and Android" still needs physical devices — logged as a capability gap, same category as Phase 1's physical-device gate.
**Owner**: executed directly this session (`commerce-pwa-engineer`'s scope); commerce-half re-verification happened post-teleport.
